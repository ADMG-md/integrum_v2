...
def test_extraction_logic():
    ...
    for line in lines:
        ...
        for keyword, loinc in extraction_service.LAB_MAP.items():
            if keyword in line_lower:
                # Same regex as in service
                match = re.search(r"(\d+[\.,]\d+|\d+)", line_lower[line_lower.find(keyword):])
                if match:
    ...
    assert any(f['code'] == "2085-9" and f['value'] == 42.8 for f in findings) # Verifies comma handling
