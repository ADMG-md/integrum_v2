...
class AnthropometryPrecisionMotor(BaseClinicalMotor):
    ...
    def compute(self, encounter: Encounter) -> AdjudicationResult:
        ...
        if whtr:
            if whtr > 0.5:
                findings.append(f"Aumento de Riesgo Cardiometabólico (WHtR {round(whtr, 2)})")
            evidence.append(ClinicalEvidence(type="Observation", code="WHtR", value=round(whtr, 2), display="R. Cintura/Altura", threshold=">0.5"))
        ...
        if whr:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code="WHR", value=round(whr, 2), display="R. Cintura/Cadera", threshold=f">{threshold}"))
        ...
        if bri:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code="BRI", value=round(bri, 2), display="Body Roundness Index", threshold=">5"))

        # Síntesis
        status = "Distribución de Grasa de Alto Riesgo" if any("Riesgo" in f or "Androide" in f for f in findings) else "Composición Antropométrica de Bajo Riesgo"
        
        explanation = "Análisis de distribución de grasa: " + ("; ".join(findings) if findings else "Métricas dentro de límites de bajo riesgo.")
