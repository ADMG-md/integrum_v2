...
class DeepMetabolicProxyMotor(BaseClinicalMotor):
    ...
    def compute(self, encounter: Encounter, context: Dict[str, Any] = None) -> AdjudicationResult:
        findings = []
        evidence = []
        ...
        if ggt:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code="GGT", value=val, display="GGT", threshold=f">{threshold}"))
            if val > threshold:
        ...
        if fer:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code="FER-001", value=val, display="Ferritina", threshold=f">{threshold}"))
            if val > threshold:
        ...
        if ua:
            val = float(ua.value)
            evidence.append(ClinicalEvidence(type="Observation", code="UA-001", value=val, display="Ácido Úrico", threshold=">6.5"))
            if val > 6.5:
        ...
        if trig and hdl and float(hdl.value) > 0:
            ...
            evidence.append(ClinicalEvidence(type="Calculation", code="TG/HDL", value=round(ratio, 2), display="Cociente TG/HDL", threshold=f">{threshold}"))
            if ratio > threshold:
                findings.append("Proxy de Dislipidemia Aterogénica (Fenotipo B / LDL denso)")
        ...
        if iso:
            findings.append("Detección de Firma Metabolómica de Resistencia a la Insulina")
        ...
        summary = " | ".join(findings) if findings else "Sin firmas de riesgo metabólico profundo"
