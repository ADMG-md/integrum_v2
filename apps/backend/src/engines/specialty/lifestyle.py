...
class Lifestyle360Motor(BaseClinicalMotor):
    ...
    def validate(self, encounter: Encounter) -> Tuple[bool, str]:
        ...
        codes_to_check = [self.CODES["AF_MINUTES"], self.CODES["STRESS_LEVEL"], self.CODES["SLEEP_HOURS"]]
        if not any(encounter.get_observation(c) for c in codes_to_check):
            return False, "Faltan pilares básicos de Estilo de Vida (AF, Estrés o Sueño)."
        return True, ""
    ...
    def compute(self, encounter: Encounter) -> AdjudicationResult:
        ...
        findings = []
        evidence = []
        explanation = "Evaluación Integral del Entorno del Paciente (Pilares Estilo de Vida). "
        confidence = 0.85
        ...
        if ais and float(ais.value) >= 6:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code=self.CODES["ATHENS_INSOMNIA"], value=ais.value, threshold=">=6", display="Escala Atenas"))
        
        if hours and float(hours.value) < 6:
            findings.append("Privación Crónica de Sueño (< 6h)")
            evidence.append(ClinicalEvidence(type="Observation", code=self.CODES["SLEEP_HOURS"], value=hours.value, threshold="<6h", display="Horas/Noche"))
            if is_insomniac: 
        ...
        if stress and float(stress.value) > 7:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code=self.CODES["STRESS_LEVEL"], value=stress.value, threshold=">7/10", display="Estrés VAS"))
        ...
        if af:
            ...
            evidence.append(ClinicalEvidence(type="Observation", code=self.CODES["AF_MINUTES"], value=af_val, display="Actividad Física (min/sem)"))
            if af_val < 150:
                findings.append("Sabotaje Sedentario (AF < 150 min/sem)")
                explanation += "No se alcanzan las metas mínimas OMS de salud metabólica. "
            elif af_val >= 300:
        ...
        return AdjudicationResult(
            calculated_value=" | ".join(findings) if findings else "Estilo de Vida en Rango de Seguridad",
